...
echo json_encode(array('notice' => "<strong>Use this short tag in the post</strong>: <input type='text' class='shortcode2' value=\"[create_table id='".$id."']\" />",'datetable'=>$product[0]->datetable,'title'=>'table:'.$product[0]->title,'cat_nos'=>$product[0]->content,'is_hide'=>$product[0]->is_hide,"h1"=>'Update Table','msg'=>$output));
}else{
