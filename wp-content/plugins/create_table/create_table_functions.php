...
function create_table_list($wpdb,$id='',$page=1,$datetable='_cs_misc_product',$search=''){
		...
		if($datetable=='_cs_misc_product'){
			$sort='catalog';
			$url='url';
			$select="`catalog`,`url`,`desc`";
			$desc='<th class="tal th">Description</th>';
		}else if($datetable=='vector'){
			$sort='vector_name';
			$url='vector_link';
			$select="`vector_link`,`vector_name`";
		}
		if(!empty($search)){
		...
		if(!empty($id)){//提供id，列出全部产品，并选中相关项
			...
			if(!empty($products2)){
				...
				if(!empty($search)){
					if($datetable=='_cs_misc_product'){
						$str1="(".$str1.") and (".$sort." like '%".$search."%' or `desc` like '%".$search."%')";
						$str2="(".$str2.") and (".$sort." like '%".$search."%' or `desc` like '%".$search."%')";
						$sql_n="SELECT count(*) FROM `{$datetable}` where {$sort} like '%{$search}%' or `desc` like '%{$search}%'";
						$sql_c="SELECT count(*) FROM `{$datetable}` where {$str1}";
					}else{
						$str1="(".$str1.") and ".$sort." like '%".$search."%'";
						$str2="(".$str2.") and ".$sort." like '%".$search."%'";
						$sql_n="SELECT count(*) FROM `{$datetable}` where {$sort} like '%{$search}%'";
						$sql_c="SELECT count(*) FROM `{$datetable}` where {$str1}";
					}
					$num=$wpdb->get_var($sql_n);
					$cou=$wpdb->get_var($sql_c);
				}else{
					$num=$wpdb->get_var("SELECT count(*) FROM `{$datetable}`");
				}
				$maxpage=ceil($num/10);
			...
			if($cou>=($page*10)){
				//var_dump($arr2);
				$products2 = $wpdb->get_results(sprintf("SELECT %s FROM `%s` where %s ORDER BY %s ASC limit %s,10",$select,$datetable,$str1,$sort,$star));
			}
			else if($cou<($page*10)&&$cou>(($page-1)*10)){
				$products2=$wpdb->get_results(sprintf("SELECT %s FROM `%s` where %s ORDER BY %s ASC limit %s,10",$select,$datetable,$str1,$sort,$star));
				$end=$page*10-$cou;
				$products = $wpdb->get_results(sprintf("SELECT %s FROM `%s` where %s ORDER BY %s ASC limit 0,%s",$select,$datetable,$str2,$sort,$end));
				//var_dump(sprintf("SELECT * FROM `%s` %s ORDER BY %s ASC limit %s,10",$datetable,$str1,$sort,$star));
			}else{
				//var_dump($str2);
				$star=$star-$cou;
				$products2='';
				$products = $wpdb->get_results(sprintf("SELECT %s FROM `%s` where %s ORDER BY %s ASC limit %s,10",$select,$datetable,$str2,$sort,$star));
			}
		}else{
			if(!empty($search)){
				//echo 1;
			...
			if($datetable=='_cs_misc_product'){
				$str.="or `desc` like '%".$search."%'";
			}
			$products = $wpdb->get_results(sprintf("SELECT %s FROM `%s` %s ORDER BY %s ASC limit %s,10",$select,$datetable,$str,$sort,$star));
			}else{
				$products = $wpdb->get_results(sprintf("SELECT %s FROM `%s` ORDER BY %s ASC limit %s,10",$select,$datetable,$sort,$star));
			}
		}
		...
		$return_string .= sprintf("
			<thead>
				<tr>
					...
					<th class=\"tal se\">%s</th>
					%s
					<th class=\"tal fo\">URL</th>
				</tr>
			</thead>
			<tfoot>
				<tr>
		...
		foreach ($products2 as $product) {
			if($datetable=='_cs_misc_product'){
				$desc='<td class="tal th">'.$product->desc.'</td>';
			}
			$return_string .= sprintf("
			<tr>
				...
				%s
				<td class=\"tal fo\"><div class=\"fl link\"><span class=\"link_icon\"></span><input type=\"text\" class=\"ln\" name=\"link[%s]\" placeholder=\"%s\" title=\"Double Click To Use This Value\" /></div></td>
			</tr>
			",$product->{$sort},$product->{$sort},$product->{$sort},$product->{$sort},$desc,$product->{$sort},$product->{$url});
		}
		}
		if(!empty($products)){
