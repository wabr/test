...
function wp_create_table() {
	global $wpdb;
		$sql =  'CREATE TABLE IF NOT EXISTS `wp_create_table` (
			...
			`is_hide` tinyint(2) DEFAULT NULL,
...
function create_table($atts) {
	...
	//查询数据库
	if(!empty($atts['id'])){
	$result = $wpdb->get_results(sprintf("SELECT * from wp_create_table where id='%s'",$atts['id']));
	if(!empty($result)){
	...
	if($table=='_cs_misc_product'){
		...
		$pro='catalog';
		$url='`url`';
		$wheresql.='where is_published=1 and ';
	...
	$products = $wpdb->get_results(sprintf("SELECT %s,%s FROM `%s` %s",$column,$url,$table,$wheresql));
	ob_start();
