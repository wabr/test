...
function get_table_list($wpdb,$page=1,$where=''){
	...
	$arr[1]=$str2;
	$arr[2]=$cou.' tables';
	$arr[3]=sprintf("select id,title,create_date from wp_create_table %s limit %s,10",$where,$star);
	return $arr;
}
...
<span class='sp'><?php echo $arr[2]; ?></span>
...
jQuery(document).ready(function($){
	$("#dt1").live('change',function(){
		...
		if(str!=search_la){
			for(key in str){
				if(key==0){
					str2="content like '%"+str[key]+"%'";
				}else{
					str2+=" and content like '%"+str[key]+"%'";
				}
			}
			where+=str2;
		}else{
