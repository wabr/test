...
if(!empty($_POST['data'])&&!empty($_POST['datetable'])){
	...
	if(!empty($_POST['dt2'])&&!empty($_POST['style'])){
		update_option($_POST['dt2'],$_POST['style']);
	}
	exit;
}
$style1=get_option('misc_style');
$style2=get_option('vector_style');
if(empty($style1)){
...
<table class="filter_table" >
	...
	<option value="1" <?php if($style1==1) echo 'selected';?> >style1</option>
	</select>
	</td>
	</tr>
</table>
...
jQuery(document).ready(function($){
	$("#dt1").live('change',function(){
		...
		if(dt1=='_cs_misc_product'){
			td+=<?php echo "'".$td1."'";?>+'</tr>';
			if("<?php echo $style1; ?>"==1)
			style='selected';
		}else if(dt1=='vector'){
			td+=<?php echo "'".$td2."'";?>+'</tr>';
			if("<?php echo $style2; ?>"==1)
			style='selected';
		}else{
			td='';
		}
		console.log(style);
		td+='<td>table style:</td><td><select class="table_style" name="table_style" id="table_style"><option value="">no style</option><option value="1" '+style+' >style1</option></select></td>';
		//console.log(td);
	...
	$(document).on("click",".save",function(){
		...
		$.ajax({
			...
			success:function(msg){console.log(msg);alert('data update success!');window.location.href='/wp-admin/admin.php?page=other_functions';}
		});
		}
	});
});
