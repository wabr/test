...
if($action=='insert'){//添加
	...
	if(!empty($title)){
		...
		$sql = sprintf("UPDATE wp_create_table SET content='%s',title='%s' WHERE id='%s';",$cat_nos,$title,$id);
	}else{
		$sql = sprintf("UPDATE wp_create_table SET content='%s' WHERE id='%s';",$cat_nos,$id);
	}
	...
	$sql = sprintf("DELETE FROM `wp_create_table` WHERE id='%s'",$id);
	$result = $wpdb->query($sql);
	if( $result !== false ){
		echo json_encode(array('status'=>'true','action'=>'delete', 'notice'=>'Products Delete.'.$sql));
	}else{
